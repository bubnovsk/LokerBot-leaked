...
let chatslist = require('./base/chatslist.json');

let settings = require('./base/settings.json'),
	base = require("./base/base.json"),
	chats = require('./base/chats.json');
...
vk.setOptions({ token: 'c554da7e11d3bb6a8c3761a9959288749bfc73e17a23e3f76c305c19071da23e1459e554aaa1ab967a701', pollingGroupId: 165571399 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[public165571399\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[public165571399\|(.*)\]/ig, '').trim();
...
const ping = utils.random(300);
	await message.send(`[📖] » Техническая информация:
     ...
     👾 || Мой проект: [pandox_bot|Пандокс Бот]
     &#4448;⚙ » OC: Bodred.host
	 &#4448;🔐 » Версия бота: 1.0
	 &#4448;❤ » Нагрузка: ${percent}%
	 &#4448;📡 » Пинг: ${ping}

	 😎 || Мой создатель - [fozzzy9293|Creator]
